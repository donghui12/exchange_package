...
应用程序集成了基于电脑码的授权验证系统，确保软件的合法使用。

### 授权流程

1. **首次启动**: 应用程序会自动检查授权状态
2. **电脑码生成**: 基于硬件信息生成唯一的电脑码
3. **授权激活**: 输入与电脑码匹配的授权码进行激活
4. **试用模式**: 支持3天试用期功能
...
项目包含授权码生成工具 `tools/license_generator.py`，用于生成测试授权码：
...
2. **选择数据包文件夹**: 选择包含pdd_goods_*.txt文件的文件夹
3. **选择输出文件夹**: 指定生成素材包的保存位置
4. **设置转换参数**: 
   - 输出间隔时间(秒)
   - 选择导出格式(格式1或格式2)
   - 是否创建ZIP压缩包
5. **开始转换**: 点击"开始转换"按钮执行转换
6. **查看进度**: 在日志区域查看转换进度和结果

### 授权管理

- **查看授权状态**: 通过"工具" -> "授权管理"菜单查看当前授权信息
- **重新激活**: 如需更换授权或重新激活，可通过授权管理对话框操作
- **试用期限**: 试用期为3天，到期后需要正式授权码
